...
from discord_bot.utils.tickets import TicketSystem
...
class BookingTicketButton(ui.Button):
    def __init__(self, ctx: Context, lang: str):
        super().__init__(label=Localization.translatable("button.yes", lang),
                         style=discord.ButtonStyle.green, custom_id=f'buy_ticket_{lang}')
...
class SeatView(ui.LayoutView):
    def __init__(self, ctx: Context, lang: str, floor: str, seat: str, disabled: bool = False):
        ...
        container.add_item(
            ui.ActionRow(BackSeatButton(ctx=ctx, lang=lang, floor=floor, disabled=disabled), BackToFloorsButton(ctx=ctx, lang=lang, disabled=disabled),
...
class CloseTicketButton(ui.Button):
    def __init__(self, ctx: Context, lang: str):
        super().__init__(label=f"{Localization.translatable('button.close_ticket', lang=lang):⠀^62}",
                         style=discord.ButtonStyle.gray, custom_id='close_ticket')
...
class ChoiseSeatView(ui.LayoutView):
    ...
    def _add_pagination(self):
        ...
        prev_button = discord.ui.Button(
            ...
            custom_id=f'seat_prev_{self.page - 1}',
        ...
        next_button = discord.ui.Button(
            emoji="➡",
            style=discord.ButtonStyle.gray,
            custom_id=f'seat_next_{self.page + 1}',
...
class BackToFloorsButton(ui.Button):
    def __init__(self, ctx: Context, lang: str, disabled: bool = False):
        super().__init__(emoji="🏠", style=discord.ButtonStyle.gray, custom_id="back_to_floors", disabled=disabled)
        self.ctx = ctx
        self.lang = lang
...
class BackSeatButton(ui.Button):
    def __init__(self, ctx: Context, lang: str, floor: str, disabled: bool = False):
        super().__init__(emoji='⬅', style=discord.ButtonStyle.gray, custom_id="back_to_floor", disabled=disabled)
        self.ctx = ctx
        self.lang = lang
        self.floor = floor
...
class SeatButton(ui.Button):
    ...
    @interaction_error_handler(logger)
    async def callback(self, interaction: discord.Interaction):
        await interaction.response.edit_message(view=SeatView(ctx=self.ctx, lang=self.lang, floor=self.floor, seat=str(interaction.data.get("custom_id"))))


class RentalSeatButton(ui.Button):
    def __init__(self, ctx: Context, lang: str, floor: str, seat: str, disabled: bool = False):
        super().__init__(label=Localization.translatable("button.book_ticket", lang), style=discord.ButtonStyle.green, custom_id=f'{seat}_rental', disabled=disabled)
        self.ctx = ctx
        self.lang = lang
        self.floor = floor
        self.seat = seat
...
class RentalRequestView(ui.View):
    def __init__(self, ctx: Context, lang: str, ticket_data: dict, disabled: bool = False):
        ...
        reject_button = discord.ui.Button(label="Отклонить", style=discord.ButtonStyle.red, custom_id='reject_seat', disabled=disabled)
        reject_button.callback = self.reject_callback

        approve_button = discord.ui.Button(label='Одобрить', style=discord.ButtonStyle.green, custom_id='approve_seat', disabled=disabled)
...
class AddPlayersModal(discord.ui.Modal):
    ...
    def __init__(self, *, ctx: Context, lang: str, floor: str, seat: str, message: discord.Message):
        super().__init__(title=Localization.translatable("modal_title.player_list", lang), timeout=None, custom_id="add_players")
