...
from discord_bot import config  # type: ignore
from discord_bot.utils.logger import BotLogger

from discord_bot.utils.sort_utils import first_number
from discord_bot.utils.ytdl_source import YTDLSource, YTDLError  # type: ignore
from discord_bot.utils.voice_utils import VoiceState, VoiceError  # type: ignore
from discord_bot.utils.song_embed import Song  # type: ignore

from discord_bot.view.player_menu import *  # type: ignore

from discord_bot.view.player_menu import Buttons
...
class Music(commands.Cog):
    ...
    async def _leave(self, ctx: commands.Context):
        ...
        if not ctx.voice_state.voice:
            return await ctx.send(embed=discord.Embed(description='Бот не подключен ни к одному голосовому каналу', color=config.COLOR), ephemeral=True)
    ...
    async def _volume(self, ctx: commands.Context, *, volume: int):
        ...
        if 0 > volume < 100:
            return await ctx.send(embed=discord.Embed(description='Громкость должна быть от 0 до 100', color=config.COLOR), ephemeral=True)
    ...
    async def _play(self, ctx: commands.Context, *, search: str):
        ...
        async with ctx.typing():
            try:
                ...
                await ctx.send(embed=discord.Embed(description='An error occurred while processing this request: {}'.format(str(e)), color=discord.Color.red), ephemeral=True)
            else:
                song = Song(source)
    ...
    async def play_file(self, ctx: commands.Context, *, filename: str):
        ...
        async with ctx.typing():
            ...
            await ctx.send(embed=discord.Embed(description=f'🎶 **Добавлено из файла**: ```{filename}```', color=config.COLOR))
    ...
    async def menu(self, ctx: commands.Context, folder: str):
        ...
        ctx.menu_state.player = await channel.send(embeds=[archive, player_embed], view=Buttons(ctx, ctx.menu_state.music_player))
